...
function recoverStaleProcessingJobs(store) {
  ...
  for (const job of store.jobs || []) {
    ...
    if (job.completedAt || job.workerCompletedAt) {
      job.status = 'completed';
      job.updatedAt = nowIso();
      job.lastRecoveryReason = 'processing_completed_state_normalized';
      continue;
    }
...
function atomicWrite(payload) {
  ensureStore();
  const temp = `${JOBS_PATH}.${process.pid}.${Date.now()}.tmp`;
...
function getNextPendingJob() {
  ...
  const sortNewestFirst = (a, b, field) => (Date.parse(b[field] || 0) || 0) - (Date.parse(a[field] || 0) || 0);
  const freshPending = pending
    .filter((job) => !job.recoveredFromStaleProcessing)
    .sort((a, b) => sortNewestFirst(a, b, 'createdAt') || sortNewestFirst(a, b, 'updatedAt'));
  if (freshPending.length) return freshPending[0];
  const recoveredPending = pending
    .filter((job) => job.recoveredFromStaleProcessing)
    .sort((a, b) => sortNewestFirst(a, b, 'recoveredAt') || sortNewestFirst(a, b, 'createdAt'));
